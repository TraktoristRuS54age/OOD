...
void TriangleDecorator::ComputePerimeter()
{
	...
	double side1 = sqrt(pow(abs(p1.x - p0.x), 2) + pow(abs(p1.y - p0.y), 2)); // Сторона между points[0] и points[1]
	double side2 = sqrt(pow(abs(p2.x - p1.x), 2) + pow(abs(p2.y - p1.y), 2)); // Сторона между points[1] и points[2]
	double side3 = sqrt(pow(abs(p2.x - p0.x), 2) + pow(abs(p2.y - p0.y), 2)); // Сторона между points[0] и points[2]
